...
def route_from_start(state: AgentState) -> str:
    ...
    return "ask_context" if state.get("context_answer", "").strip() else "classification"
...
def route_after_pre_validate(state: AgentState) -> str:
    """일정 유효성과 분해 필요성에 따라 질문, 계획, 성공 또는 실패로 이동한다."""
...
def create_graph():
    ...
    builder.add_conditional_edges(
        START,
        route_from_start,
        {"ask_context": "ask_context", "classification": "classification"},
    )
    builder.add_conditional_edges(
        "classification",
    ...
    builder.add_conditional_edges(
        ...
        {"classification": "classification", "pre_validate": "pre_validate", "end": END},
    )
    builder.add_conditional_edges(
        "pre_validate",
        route_after_pre_validate,
        {"ask_context": "ask_context", "plan": "plan", "output": "output", "fallback": "fallback"},
    )
