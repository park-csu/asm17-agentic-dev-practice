...
현재 API 요청 모델의 `classification_retry`, `plan_retry`, `detail_with_context`, `context_answer`는 정식 외부 API 계약이 아니라 LangGraph 프로토타입의 상태 전달 필드입니다. 서버 상태 저장이 아직 없기 때문에 추가 질문과 재시도 흐름을 요청/응답으로 이어가기 위해 유지합니다.
