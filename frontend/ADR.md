...
## ADR-014: 캘린더 컨테이너 리사이즈를 FullCalendar에 직접 알린다

사이드바 토글은 `.app-shell`의 컬럼 폭을 애니메이션으로 바꾸지만 브라우저 창 크기 자체는 변하지 않는다. FullCalendar는 창 리사이즈가 아닌 부모 컨테이너 폭 변경을 즉시 감지하지 못해 scrollgrid 폭과 이벤트 배치가 어긋날 수 있다.

결정:
- `.calendar-surface`에 `ResizeObserver`를 연결한다.
- 컨테이너 크기가 변하면 FullCalendar API의 `updateSize()`를 호출해 내부 레이아웃을 다시 계산한다.
- 팝오버가 열려 있는 상태에서 캘린더 위치가 바뀌면 anchor 좌표를 다시 계산한다.
- 팝오버 위치 점프를 막기 위해 리사이즈 중에도 anchor를 `null`로 되돌리지 않는다.
- anchor가 아직 없으면 팝오버를 fallback 위치에 먼저 렌더하지 않는다.

## ADR-015: 추가 맥락 답변 입력값은 일정별로 분리한다

`needs_question` 팝오버의 답변 textarea를 전역 문자열 하나로 관리하면, 이전 일정에 입력한 답변 초안이 새 일정 또는 다른 일정의 질문 폼에 그대로 보인다.

결정:
- 답변 초안은 `scheduleId`별 map으로 관리한다.
- 답변 제출은 선택된 일정이 아니라 현재 열린 팝오버 일정의 continuation을 기준으로 수행한다.
- 일정 삭제, 로그아웃, 스트림 완료 시 해당 일정의 답변 초안을 제거한다.
