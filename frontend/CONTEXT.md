...
- 3열 앱 셸을 사용한다.
- 왼쪽 사이드바는 접었다 펼 수 있다 (토글 버튼으로 `0px ↔ 280px` 전환). 기본값은 펼침.
- 왼쪽은 일정 목록을 보여주며, 일정 항목 클릭은 선택만 수행한다.
- `수정`, `삭제` 액션은 사이드바가 아닌 캘린더 이벤트 클릭 시 나타나는 팝오버에 배치한다.
- 팝오버에는 일정 제목, 시간, 위치, 수정/삭제 버튼, 생성된 task 목록(체크박스 포함)이 표시된다.
- 가운데는 FullCalendar 기반 캘린더를 둔다. `slotMinTime="00:00:00"`, `slotMaxTime="24:00:00"`으로 24시간 전체를 표시한다.
- 오른쪽은 선택 일정 결과 패널이 아니라 날짜별 task 보드로 구성한다.
- `needs_question` 답변 폼은 캘린더 이벤트 클릭 팝오버 안에서 표시한다.
- 팝오버 위치는 `eventDidMount`에서 이벤트 DOM 요소에 `data-schedule-id`를 설정하고, `popoverScheduleId` 변경 시 50ms 지연 후 DOM 좌표를 읽어 결정한다.
- 모바일 또는 hover가 불안정한 환경에서는 이벤트 hover 팝오버를 비활성화한다.
- 앱 상단 제목은 항상 `TaskPilot`으로 고정한다.
